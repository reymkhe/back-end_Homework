...
public class AndGrepTest {
    ...
    /**
     * Test for AndGrep function, grep with case-insensitive filters
     * @throws IOException when reader is not exist
     */
    @Test
    public void caseInsensitiveFilterAndGrep() throws IOException {
        AndGrep andGrep = new AndGrep ("MY", "life");
        when(mockReader.read()).thenReturn(109,121,32,119,111,114,108,100,10,109,121,32,76,73,70,69,10,72,101,76,76,111,-1);
        List<String> expectedSource = new ArrayList<String>() {{
            add("my LIFE");
        }};
        Assert.assertArrayEquals(expectedSource.toArray(), andGrep.doGrep(mockReader).toArray());
